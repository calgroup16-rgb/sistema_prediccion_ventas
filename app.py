...
# --- BARRA LATERAL CON MENÚ NAVEGADOR ---
st.sidebar.header("Navegación del Sistema")

menu_seccion = st.sidebar.radio(
    "Seleccione la sección:",
    [
        "1. Base de Datos Principal & Predicciones",
        "2. Informe Bimensual",
        "3. Filtro / Lista Propia (Opcional)"
    ]
)
...
if 'df_ventas' in st.session_state and st.session_state['df_ventas'] is not None:
    ...
    # --- SECCIÓN 1: BASE PRINCIPAL Y PREDICCIONES COMERCIALES ---
    if menu_seccion == "1. Base de Datos Principal & Predicciones":
        # PESTAÑAS DE PREDICCIÓN Y ANÁLISIS
        ...
        with tab1:
            if not df_global.empty:
                ...
                m1.metric("Clientes Evaluados", df_global['CLIENTE'].nunique())
                m2.metric("Ventas Acumuladas ($)", f"${df_global['VALOR_VENTA'].sum():,.2f}")
                m3.metric("Transacciones Analizadas", f"{len(df_global):,}")
                ...
                hist_compra = df_global.groupby(['CLIENTE', 'PRODUCTO'])['VALOR_VENTA'].sum().reset_index()
                reciente_compra = df_global[df_global['ANIO'] == max_anio].groupby(['CLIENTE', 'PRODUCTO'])['VALOR_VENTA'].sum().reset_index()
        ...
        with tab2:
            ...
            clientes_2026 = set(df_global[df_global['ANIO'] == 2026]['CLIENTE'].unique())
            df_hist = df_global[(df_global['ANIO'] >= 2019) & (df_global['ANIO'] <= 2025)]
        ...
        with tab3:
            ...
            es_cliente = df_global['CLIENTE'].str.contains(patron, case=False, na=False)
            es_prod = df_global['PRODUCTO'].str.contains(patron, case=False, na=False)

            df_animal = df_global[es_cliente | es_prod]
            ...
            if not df_animal.empty:
                ...
                with col2:
                    ...
                    for cli in top_animal['NOMBRE SN']:
                        comprados = set(df_global[df_global['CLIENTE'] == cli]['PRODUCTO'].unique())
    ...
    # --- SECCIÓN 2: INFORME BIMENSUAL ---
    elif menu_seccion == "2. Informe Bimensual":
        render_modulo_informe(df_global)

    # --- SECCIÓN 3: FILTRO / LISTA PROPIA (OPCIONAL) ---
    elif menu_seccion == "3. Filtro / Lista Propia (Opcional)":
        st.header("🎯 Filtro y Generación de Lista Propia de Clientes")
        ...
        if archivo_lista_clientes is not None:
            ...
            st.success(f"Análisis enfocado en {len(lista_nombres)} cliente(s) de la lista subida.")
            ...
            st.success(f"Análisis enfocado en {len(clientes_manuales)} cliente(s) seleccionado(s).")
            st.dataframe(df_trabajo, use_container_width=True)
            
        else:
            st.info("Utilice los controles de la barra lateral para filtrar clientes o subir un listado personalizado.")
